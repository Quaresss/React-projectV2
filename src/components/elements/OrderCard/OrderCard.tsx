    ...
    <div className={style.card}>
      <div className={style.card__top}>
        <div className={style.card__detail}>
          <div className={style.card__detailItem}>Дата заказа {orderDate}</div>
          <div className={style.card__detailItem}>Заказ № {orderNumber}</div>
        </div>
        <div className={cn(style.card__detail, style.card__detail_theme)}>
          <div className={style.card__detailItem}> К оплате: &#36; {totalAmount.toFixed(2)}</div>
          <div className={style.card__detailItem}>Кол-во: {totalCount} шт.</div>
        </div>
      </div>
        ...
        <div className={style.card__bottomText}>
          <div className={style.card__address}>
            <span className={style.card__delivery}>
              {deliveryType === DeliveryType.DELIVERY ? 'Доставка: ' : 'Забрать в ресторане: '}
            </span>
